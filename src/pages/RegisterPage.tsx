...
import { Sparkles, Mail, Github, ArrowRight, Eye, EyeOff, User, Check, Building2, Briefcase, Phone, BrainCircuit } from "lucide-react";
        ...
        <div className="w-full max-w-md">
                ...
                <div className="mt-2 space-y-1">
                  {passwordChecks.map((c) => (
                    <div key={c.label} className="flex items-center gap-2 text-xs">
                      <Check className={`w-3 h-3 ${c.met ? "text-secondary-dark" : "text-muted-foreground/40"}`} />
                      <span className={c.met ? "text-secondary-dark" : "text-muted-foreground/60"}>{c.label}</span>
                    </div>
                  ))}
                </div>
              )}
          ...
          <Link to="/login">
            <p className="text-center text-sm text-muted-foreground mt-6">
              Already have an account? <span className="text-secondary-dark hover:underline font-medium">Sign in</span>
            </p>
          </Link>
        </div>
      </div>
