...
const aiActions = [
  { icon: RefreshCw, label: "Regenerate Section", description: "Rewrite this section from scratch" },
  { icon: Sparkles, label: "Improve Section", description: "Enhance clarity and detail" },
  { icon: Expand, label: "Expand Technical Depth", description: "Add more technical specifics" },
  { icon: BarChart3, label: "Add Competitive Analysis", description: "Compare with market solutions" },
  { icon: Wand2, label: "Regenerate Document", description: "Recreate the entire document" },
];
      ...
      <div className="w-64 border-r border-border bg-card/30 p-4 overflow-y-auto shrink-0">
        ...
        <div className="space-y-1">
          {sections.map((s) => (
            <button
              key={s.id}
              onClick={() => { setActiveSection(s.id); setEditingContent(null); }}
              className={cn(
            ...
            >
              {approvedSections.has(s.id) && <Check className="w-3.5 h-3.5 text-emerald-400 shrink-0" />}
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                className={approvedSections.has(currentSection.id) ? "border-emerald-400/50 text-emerald-400" : "gradient-primary text-primary-foreground border-0"}
      ...
      <div className="w-72 border-l border-border bg-card/30 p-4 overflow-y-auto shrink-0">
        ...
        <div className="space-y-2">
          {aiActions.map((action) => (
            <button
              key={action.label}
              className="w-full flex items-start gap-3 p-3 rounded-lg border border-border bg-secondary/20 hover:bg-secondary/40 hover:border-primary/30 transition-all text-left group"
            >
